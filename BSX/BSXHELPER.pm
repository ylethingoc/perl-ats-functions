...
# NAME
#
# SonusQA::BSX::BSXHELPER - Perl module for BSX interaction
#
# DESCRIPTION
#	This BSXHELPER package contains various subroutines that assists with BSX related interactions.
...
#
#	sub getProcessfromBsxApplList()
#       sub sortProcessesforStartup()
#       sub checkHangingCallLegsinCCSW()
#       sub restartProcesses()
#       sub checkRequiredBsxProcessesRunning()
#       sub makeRequiredBsxApplProcessRunning()
...
=head1 getProcessfromBsxApplList()

DESCRIPTION:

 The function is used to specific process(s) from list
 i.e. CCSW/ADVLR/PCMGR/MSGHDLR/IPMH/CDRCP/LOGCM - process type

ARGUMENTS:

1. The Application process type (CCSW/ADVLR/PCMGR/MSGHDLR/IPMH/CDRCP/LOGCM)
...
EXAMPLE:
   ...
   # get the list of CCSW type process(s)
    my ($result, @list);
    ($result, @list) = $BsxObj->getProcessfromBsxApplList('CCSW', \@BsxApplList);
    unless ( $result ) {
        $TESTSUITE->{$TestId}->{METADATA} .= "getProcessfromBsxApplList() - Failed";
    ...
    ($result, $ccsw01, $ccsw02) = $BsxObj->getProcessfromBsxApplList('CCSW', \@BsxApplList);
    unless ( $result ) {
        $TESTSUITE->{$TestId}->{METADATA} .= "getProcessfromBsxApplList() - Failed";
        printFailTest (__PACKAGE__, $TestId, "$TESTSUITE->{$TestId}->{METADATA}");
        return 0;
...
sub getProcessfromBsxApplList {
#################################################
    my ($self, $processType, $bsxApplList) = @_;
    my $subName = 'getProcessfromBsxApplList()';
    ...
    unless ( defined($processType) ) {
        $logger->error(__PACKAGE__ . ".$subName:  No BSX process type specified." );
        $logger->debug(__PACKAGE__ . ".$subName: <-- Leaving Sub [0]");
        return $result;
    }

    unless ( defined($bsxApplList) ) {
        $logger->error(__PACKAGE__ . ".$subName:  No BSX application list reference specified." );
        $logger->debug(__PACKAGE__ . ".$subName: <-- Leaving Sub [0]");
        return $result;
    }
    ...
    if ( @processList ) {
        ...
        $logger->error(__PACKAGE__ . ".$subName:  No BSX process ($processType) application found in list  specified." );
    }
...
DESCRIPTION:
 ...
 i.e. order of sorting is ADVLR IPMH PCMGR MSGHDLR CCSW LOGCM CDRCP processes
...
sub sortProcessesforStartup {
    ...
    unless ( defined($processList) ) {
        $logger->error(__PACKAGE__ . ".$subName:  No BSX process list specified." );
        $logger->debug(__PACKAGE__ . ".$subName: <-- Leaving Sub [0]");
        return $completeFlag;
    }
    ...
    my @order = qw(ADVLR IPMH PCMGR MSGHDLR CCSW LOGCM CDRCP);
...
EXAMPLE:
    ...
    my ( $result, $ccswList ) = $BsxObj->checkHangingCallLegsinCCSW( \@ccsw );
    unless ( $result ) {
        if ( defined($ccswList) ) {
            $TESTSUITE->{$TestId}->{METADATA} .= "we have hanging call leg(s) in:--\n@{$ccswList}";
            $BsxObj->restartProcesses( $ccswList );
        }
        else {
...
sub checkHangingCallLegsinCCSW {
    ...
    unless ( defined($processList) ) {
        $logger->error(__PACKAGE__ . ".$subName:  No BSX CCSW(s) specified." );
        $logger->debug(__PACKAGE__ . ".$subName: <-- Leaving Sub [0]");
        return $result;
    }
...
EXAMPLE:
    ...
    unless ( $BsxObj->restartProcesses( $ccswList ) ) {
...
=head1 checkRequiredBsxProcessesRunning()
...
EXAMPLE:
    ...
    my %REQUIREDBSXPROCESSES = (
        CCSW    => { ACTIVE => 1, BACKUP => 0 },
        ADVLR   => { ACTIVE => 1, BACKUP => 0 },
        PCMGR   => { ACTIVE => 1, BACKUP => 0 },
        MSGHDLR => { ACTIVE => 1, BACKUP => 0 },
    # Process(s) not used in this feature
    #   IPMH    => { ACTIVE => 1, BACKUP => 0 },
    #   CDRCP   => { ACTIVE => 1, BACKUP => 0 },
    );

    # Check required BSX processes are in RUNNING state
    my ($result, $appList) = $BsxObj->checkRequiredBsxProcessesRunning( \%REQUIREDBSXPROCESSES );
    unless ( $result ) {
        $logger->error(__PACKAGE__ . " ======: Could not find required Bsx processes in RUNNING state"); 
        return 0;
    }
    $logger->debug(__PACKAGE__ . " ======: Could find required Bsx processes in RUNNING state:--\n@{$appList}");

=cut

#################################################
# checkRequiredBsxProcessesRunning()
#################################################
sub checkRequiredBsxProcessesRunning {
    my ($self, $hrefRequiredProcesses) = @_;
    my $subName = 'checkRequiredBsxProcessesRunning()';
    my $logger        = Log::Log4perl->get_logger(__PACKAGE__ . ".$subName");
    $logger->debug(__PACKAGE__ . ".$subName: --> Entered Sub");

    unless ( defined($hrefRequiredProcesses) ) {
        $logger->error(__PACKAGE__ . ".$subName:  No BSX process(s) specified." );
        $logger->debug(__PACKAGE__ . ".$subName: <-- Leaving Sub [0]");
        return 0;
    }
...
print(__PACKAGE__ . ".$subName: requiredProcesses=$requiredProcesses, count=$count, processType=$processType, ActiveBackup=$ActiveBackup\n");
                unless ( $requiredProcesses == $count ) {
                    $logger->debug(__PACKAGE__ . ".$subName:  FAILED - for \'$processType\', $ActiveBackup - required process(s) \($requiredProcesses\) but found only $count:--\n@appList");
print(__PACKAGE__ . ".$subName: errorFlag=$errorFlag\n");
                    $errorFlag = 1;
                }
                $logger->debug(__PACKAGE__ . ".$subName:  SUCCESS: for \'$processType\', $ActiveBackup - required process(s) \($requiredProcesses\) found:--\n@appList");
            }
        }

print(__PACKAGE__ . ".$subName: processTypeFoundFlag=$processTypeFoundFlag, errorFlag=$errorFlag\n");
    ...
    if (@stopAppList) {
        ...
        $logger->warn(__PACKAGE__ . ".$subName:  Process(s) not in BSX required list, being stopped are:--\n@list");
        $logger->warn(__PACKAGE__ . ".$subName:  ******************************************************");
...
=head1 makeRequiredBsxApplProcessRunning()
...
EXAMPLE:
    ...
    unless ( $BsxObj->makeRequiredBsxApplProcessRunning(
                                                        '-applications' => \@BsxApplList,
                                                        '-timeout'      => 20, # Optional
                                                        '-retry'        => 3,  # Optional
                                                        '-sleeptime'    => 30, # Optional
                                                       ) ) {
        $TESTSUITE->{$TestId}->{METADATA} .= "Cannot have required BSX applications in RUNNING state.";
        printFailTest (__PACKAGE__, $TestId, "$TESTSUITE->{$TestId}->{METADATA}");
        $logger->error(__PACKAGE__ . ".$TestId:  Cannot have required BSX applications in RUNNING state.");
        return 0;
    }
...
#################################################
# makeRequiredBsxApplProcessRunning()
#################################################
sub makeRequiredBsxApplProcessRunning {
    my ($self, %args) = @_;
    my $subName = 'makeRequiredBsxApplProcessRunning()';
    my $logger        = Log::Log4perl->get_logger(__PACKAGE__ . ".$subName");
    $logger->debug(__PACKAGE__ . ".$subName: --> Entered Sub");
    ...
    for ( my $loopCount = 1; $loopCount <= $a{'-retry'}; $loopCount++ ){
            ...
            foreach ( @AOH_Data ) {
                ...
                if ( $_->{$AppName} eq $applName ) {
                    ...
                    if ( $_->{$State} =~ /RUNNING/ ) {
                        ...
                        my $cmd;
                        if ( $_->{$AppName} =~ /PCMGR/ ) {
                            # May be the MSGHDLR is up & running before the PCMGR is up.
                            # - STOP all the MSGHDLR processes
                            # - START the PCMGR process
                            # - START all the MSGHDLR processes
                            my ($retVal, @list) = $self->getProcessfromBsxApplList('MSGHDLR', \@appList);
                            unless ( $retVal ) {
                                $errorFlag = 1;
                                last;
                            }
                            $logger->debug(__PACKAGE__ . ".$subName: list of MSGHDLR processes found:--\n@list");
        ...
        if ( $applCountRunningState == ($#appList + 1) ) {
            $logger->debug(__PACKAGE__ . ".$subName:  All BSX Applications($applCountRunningState) required are in RUNNING state.");
            last;
        }
...
EXAMPLE:
    ...
    my ($result, $href_processStates) = $BsxObj->validateConsoleCommand (
                                          '-cmd'        => 'showprocs all',
...
EXAMPLE:
   ...
   %validate   = ( # Process(s) to validate
    ...
    my ($result, $aref_processStates) = $BsxObj->checkConsoleCommand (
                                          '-cmd'        => 'showprocs all',
                                          '-keyColName' => 'AppName',
                                          '-valColName' => 'State',
                                          '-validate'   => \%validate,
