...
SonusQA::MSX - Perl module for Sonus Networks MSX interaction

SYSOPSIS

 use ATS; # This is the base class for Automated Testing Structure

 my $MsxObj = SonusQA::MSX->new(
                             #REQUIRED PARAMETERS
...
our @ISA = qw(SonusQA::Base SonusQA::MSX::MSXHELPER);

# INITIALIZATION ROUTINES FOR MSX
# -------------------------------
...
DESCRIPTION:
 ...
 The function is the generic function to issue a command to the MSX.
...
DESCRIPTION:
 ...
 The function is a wrapper around execCmd for the MSX linux shell.
...
DESCRIPTION:
 ...
 The function is a wrapper around execCmd that also parses the output to look for MSX TL1 specific strings: 'COMPLD' and 'DENY'. It will then return 1 or 0 depending on this. In the case of timeout 0 is returned. The TL1 output from the command is then only accessible from $self->{CMDRESULTS}. The idea of this function is to remove the parsing for 'COMPLD' or 'DENY' from every TL1 command call. 
...
DESCRIPTION:
 ...
 The function is to establish MSX TL1 session.
...
DESCRIPTION:
 ...
 The function is to leave MSX TL1 session.
 It will then return 1 or 0 depending on this. In the case of timeout 0 is returned.
...
DESCRIPTION:
 ...
 The function is to establish MSX console session.
 It will then return 1 or 0 depending on this. In the case of timeout 0 is returned.
...
EXAMPLE:
    ...
    unless ( $MsxObj->enterConsoleSession ( 
                                     -login    => $ConsoleUserId,
...
DESCRIPTION:
 ...
 The function is to leave MSX console session.
 It will then return 1 or 0 depending on this. In the case of timeout 0 is returned.
...
EXAMPLE:
    ...
    unless ( $MsxObj->leaveConsoleSession() ) {
...
DESCRIPTION:
 ...
 The function is a wrapper around execCmd that also parses the output to look for MSX CONSOLE specific strings: 'rows received' and 'DENY'. It will then return 1 or 0 depending on this. In the case of timeout 0 is returned. The CONSOLE output from the command is then only accessible from $self->{CMDRESULTS}. The idea of this function is to remove the parsing for 'COMPLD' or 'DENY' from every CONSOLE command call. 
...
sub _enterSession {
    ...
    # Check Mandatory Parameters
    foreach ( 'login', 'password', 'session' ) {
        unless ( defined ( $args{"-$_"} ) ) {
            $logger->error(__PACKAGE__ . ".$subName:  The mandatory argument for -$_ has not been specified or is blank.");
            $logger->debug(__PACKAGE__ . ".$subName: <-- Leaving Sub [0]");
            return 0;
    ...
    if ( $match =~ m/[P|p]assword:/ ) {
        ...
        if ( $args{"-session"} == $sessionType{TL1} ) {
            ...
            if ( $match =~ m/LOGIN_CTAG COMPLD/i ) {
                ...
                $logger->debug(__PACKAGE__ . ".$subName:  Password used \($password\) accepted for \'TL1 Session\'");
            ...
            if ( $match =~ m/Entering console mode/i ) {
                ...
                $logger->debug(__PACKAGE__ . ".$subName:  Password used \($password\) accepted for \'CONSOLE Session\'");
